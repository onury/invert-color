import type { BlackWhite, HexColor, RGB, RgbArray } from '../src/index.js';
import invert, { invert as namedInvert } from '../src/index.js';
...
const CUSTOM_BW_COLORS: BlackWhite = { black: CUSTOM_BLACK, white: CUSTOM_WHITE };

describe('invert-color', () => {
  test('matches Adobe Photoshop CC inverted colors', () => {
    //            ORIGINAL            PHOTOSHOP
    //            COLORS              INVERTED
    // ---------------------------------------------
  ...
  test('accepts [r, g, b] arrays and { r, g, b } objects', () => {
  ...
  test('amplifies to black or white with `bw: true`', () => {
  ...
  test('amplifies to custom black and white colors', () => {
  ...
  test('supports true / false / object for the `bw` parameter', () => {
  ...
  test('inverts to/from array/object as B/W (and does not mutate input)', () => {
    // hex → array
    ...
    // input not mutated
    expect(A_WHITE).toEqual([255, 255, 255]);
    expect(O_WHITE).toEqual({ r: 255, g: 255, b: 255 });
  });

  test('inverts to/from array/object as custom B/W', () => {
    // hex → array
  ...
  test('honors a custom luminance threshold', () => {
    const color = '#929292';
    const at = (t: number) =>
      invert(color, { black: CUSTOM_BLACK, white: CUSTOM_WHITE, threshold: t });
    const withDefault = at(defaultThreshold);
    expect(withDefault).toEqual(invert(color, CUSTOM_BW_COLORS)); // no threshold defined
    expect(withDefault).toEqual(CUSTOM_BLACK);
    expect(at(0)).toEqual(CUSTOM_BLACK);
    expect(at(0.1)).toEqual(CUSTOM_BLACK);
    expect(at(0.2)).toEqual(CUSTOM_BLACK);
    expect(at(0.24)).toEqual(CUSTOM_BLACK);
    expect(at(0.28)).toEqual(CUSTOM_BLACK);
    expect(at(0.3)).toEqual(CUSTOM_WHITE);
    expect(at(0.5)).toEqual(CUSTOM_WHITE);
  ...
  test('applies the luminance boundary and threshold exactly', () => {
    const BW = { black: '#000000', white: '#ffffff' };
    // luminance(0,0,0) === 0; with threshold 0 the comparison is strictly `> 0`,
    // so black(0) stays on the white side — pins `>` (not `>=`) and the sRGB knee.
    expect(invert([0, 0, 0], { ...BW, threshold: 0 })).toEqual('#ffffff');
    // A near-black channel sits in the linear (x/12.92) segment: its true
    // luminance (~0.0003) is below 0.0005, so it pairs with white — pins the
    // divide and the linear-vs-power branch.
    expect(invert([1, 1, 1], { ...BW, threshold: 0.0005 })).toEqual('#ffffff');
  });

  test('throws on invalid HEX', () => {
    expect(() => invert('xyz')).toThrow('Invalid HEX color: "xyz"');
  ...
  test('throws on other invalid color values', () => {
    expect(() => invert(null as unknown as HexColor, true)).toThrow('Invalid color value');
  });

  test('does not throw for valid HEX with or without a `#` prefix', () => {
  ...
  test('exposes the documented surface', () => {
    ...
    expect(invert.asRgbObject).toBe(invert.asRGB);
    expect(invert.asRgbObject('#fff')).toEqual({ r: 0, g: 0, b: 0 });
    expect(typeof invert.defaultThreshold).toEqual('number');
  });

  test('is available as both a default and a named export', () => {
    expect(namedInvert).toBe(invert);
    expect(namedInvert('#000')).toEqual('#ffffff');
  });
});
